...
interface UseBoursesResult {
  ...
  meta: { total: number; returned: number; matched?: number; excluded?: number; countries?: string[]; sources?: { total: number; curated: number; catalog: number; china: number; synced: number } } | null;
  loading: boolean;
  error: string | null;
  refetch: () => void;
}
