...
interface UseBoursesResult {
  ...
  meta: { total: number; returned: number; matched?: number; excluded?: number; countries?: string[]; sources?: { total: number; curated: number; catalog: number; china: number; france: number; germany: number; belgium: number; canada: number; synced: number } } | null;
  loading: boolean;
  error: string | null;
  refetch: () => void;
}
